...
export function PostCard({
  ...
  const handleShare = async () => {
    const shareUrl = `${window.location.origin}/edu/post/${post.id}`;
    try {
      if (navigator.share) {
  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg active:scale-[0.99] dark:border-slate-800 dark:bg-slate-900">
      ...
      >
        ...
        {!hasMedia && (
          <div className="flex flex-col items-center gap-2 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/80 text-[#1E7F43] shadow">
              <MediaIcon className="h-6 w-6" />
            </div>
            <div className="text-xs uppercase tracking-[0.3em] text-slate-500">{post.media.label}</div>
            <p className="max-w-xs text-sm text-slate-600">{post.media.description}</p>
          </div>
        )}
      ...
      {/* Comment preview */}
      <div className="px-5 pb-4 text-xs text-slate-600 dark:text-slate-300">
        {(commentCount > 0 || commentList.length > 0) && (
          <button
            type="button"
            onClick={toggleComments}
            className="mb-2 inline-flex text-[11px] font-semibold text-slate-400 hover:text-[#1E7F43]"
        ...
        {!commentLoading && !commentError && commentList.length > 0 && (
          <div className={`space-y-3 ${commentsExpanded ? "max-h-56 overflow-y-auto pr-1" : ""}`}>
