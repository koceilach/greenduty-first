-- Add pinned product context to marketplace chats and upgrade create/find RPC.

alter table public.marketplace_conversations
  add column if not exists pinned_item_id uuid references public.marketplace_items(id) on delete set null,
  add column if not exists pinned_item_title text,
  add column if not exists pinned_item_image_url text,
  add column if not exists pinned_item_price_dzd numeric(12, 2),
  add column if not exists pinned_by uuid references public.marketplace_profiles(id) on delete set null,
  add column if not exists pinned_at timestamptz;

create index if not exists marketplace_conversations_pinned_item_id_idx
  on public.marketplace_conversations (pinned_item_id);

drop function if exists public.marketplace_find_or_create_direct_conversation(uuid);
drop function if exists public.marketplace_find_or_create_direct_conversation(uuid, uuid);
